...
use dlutos_hal_traits::mm::Memory;
use dlutos_mm::{
...
pub unsafe extern "C" fn riscv64_start(hart_id: usize) -> ! {
    ...
    unsafe extern "Rust" {
        fn _dlutos_hal_main(_: BootStrapData) -> !;
    }
    ...
    unsafe {
        _dlutos_hal_main(bootstrap_data);
    }
}
...
fn ap_entry(hart_id: usize, stack_bottom: PAddr) -> ! {
    ...
    unsafe extern "Rust" {
        fn _dlutos_hal_ap_main(stack_range: PRange) -> !;
    }
    ...
    unsafe {
        _dlutos_hal_ap_main(stack_range);
    }
}
